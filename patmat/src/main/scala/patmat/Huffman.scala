...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def addx(x: Char, chars: List[(Char, Int)]): List[(Char, Int)] = chars match {
      case Nil => List((x, 1))
      case (c, n) :: cs => if (c == x) (c, n + 1) :: cs else (c, n) :: addx(x, cs)
    }
    def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = chars match {
      case Nil => acc
      case x :: xs => timesAcc(xs, addx(x, acc))
    }
    timesAcc(chars, Nil)
...
//    pack(chars) map (ys => (ys.head, ys.length))
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    freqs match {
      case Nil => Nil
      case (c, n) :: cs => insert(Leaf(c, n), makeOrderedLeafList(cs))
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAcc(tre: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = (bits, tre) match {
      case (_, Leaf(c, w)) => decodeAcc(tree, bits, acc ::: List(c))
      case (Nil, _) => acc
      case (x :: xs, Fork(l, r, cs, w)) =>
        if (x == 0) decodeAcc(l, xs, acc) else decodeAcc(r, xs, acc)
      case (_, _) => throw new Error("something went wrong")
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeAcc(tre: CodeTree, text: List[Char], acc: List[Bit]): List[Bit] = (text, tre) match {
      case (Nil, _) => acc
      case (x :: xs, Leaf(c, w)) => if (x == c) encodeAcc(tree, xs, acc) else Nil
      case (_, Fork(l, r, cs, w)) =>
        encodeAcc(l, text, acc ::: List(0)) ::: encodeAcc(r, text, acc ::: List(1))
      case (_, _) => throw new Error("something went wrong")
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case Nil => Nil
    case t :: ts => if (char == t._1) t._2 else codeBits(ts)(char)
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(tre: CodeTree, acc: List[Bit]): CodeTable = tre match {
      case Leaf(c, w) => List((c, acc))
      case Fork(l, r, cs, w) =>
        mergeCodeTables(convertAcc(l, acc ::: List(0)), convertAcc(r, acc ::: List(1)))
    }
    convertAcc(tree, Nil)
  }
  /**
